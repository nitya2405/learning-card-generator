...
A TypeScript + Fastify backend that calls the Gemini API to generate structured, schema-validated learning cards for school students (CBSE/ICSE/State boards). Built as a PopGamma take-home assignment.
...
The Gemini response is validated against a strict Zod schema. If validation fails, the service retries once with the validation error appended as a correction instruction.
...
Open `.env` and replace `your_gemini_api_key_here` with your actual Gemini API key from [Google AI Studio](https://aistudio.google.com/app/apikey).
...
- No caching: identical `(board, grade, concept)` requests hit Gemini every time. A Redis or in-memory LRU cache would eliminate redundant API calls.
- No test suite: the Zod schemas act as executable specs but there are no recorded fixture tests against the Gemini response.
- Temperature is set to 0.4 globally; it's not tunable per request, which limits diversity for repeated requests on the same concept.
...
- **Streaming responses** — use Gemini's streaming API and server-sent events so the client sees the card sections as they arrive rather than waiting for the full JSON.
...
- **Caching** — Redis cache keyed by `sha256(board + grade + concept)` with a 24-hour TTL to avoid redundant Gemini calls.
- **Test suite** — record a handful of Gemini responses as fixtures and run Zod validation against them in CI, plus integration tests against a mock HTTP server.
