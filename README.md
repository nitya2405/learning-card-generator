...
### Provider choice: Groq + Llama 3.3 70B

The initial implementation targeted Google Gemini, then Anthropic Claude. Gemini's free tier returned `limit: 0` on all quota metrics for certain Google Cloud project configurations. Anthropic requires paid credits (no free tier). Groq provides a genuinely free tier — 30 RPM, 14,400 RPD — with no credit card required, making it accessible for development and evaluation without friction.
